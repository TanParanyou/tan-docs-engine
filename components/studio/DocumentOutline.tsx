      ...
      <div className="p-2.5 border-b border-slate-200 bg-white">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
        {filteredHeadings.length === 0 ? (
          ...
          filteredHeadings.map((h) => {
            const indentClass =
              h.level === 1
                ? "pl-2 font-bold text-slate-900 bg-slate-50/50"
                : h.level === 2
                ? "pl-4 text-slate-800 font-semibold"
                : h.level === 3
                ? "pl-6 text-slate-600 font-medium"
            ...
            return (
              <button
                ...
                className={`w-full group text-left py-1.5 pr-2 rounded-lg hover:bg-blue-50 hover:text-blue-700 transition-colors flex items-center justify-between text-xs ${indentClass}`}
              >
